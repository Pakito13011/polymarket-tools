...
N_FETCH = 3000
LIVE_LO = 0.05        # ne traiter que les marches vivants (non resolus)
LIVE_HI = 0.95
SIGMA_DAY = 1.3       # biais de prevision partage sur la journee (deg C)
SIGMA_HOUR = 0.4      # bruit horaire residuel
N_MC = 2000
EDGE_MIN = 0.08
KELLY_FRAC = 0.5
KELLY_CAP = 0.10      # plafond de mise par pari (fraction du budget)
BUDGET = 500.0
STATE = "wxbets.json"
...
S = requests.Session()
S.headers.update({"User-Agent": "pm-wxval2/1.0"})
...
def hourly(lat, lon):
    ...
    d = jget(WX, latitude=lat, longitude=lon, hourly="temperature_2m",
             forecast_days=4, timezone="auto")
    off = d.get("utc_offset_seconds", 0)
    h = d.get("hourly", {})
    res = (off, h.get("time", []), h.get("temperature_2m", []))
...
def day_samples(lat, lon, date_str, kind):
    ...
    off, times, temps = hourly(lat, lon)
    now_local = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(seconds=off)
    obs = []
    fut = []
    for t, v in zip(times, temps):
        if v is None or not t.startswith(date_str):
            continue
        try:
            tt = datetime.fromisoformat(t)
        except Exception:
            continue
        (obs if tt <= now_local else fut).append(v)
    base = None
    if obs:
        base = max(obs) if kind == "high" else min(obs)
    res = []
    for _ in range(N_MC):
        ...
        if kind == "high":
            ...
            for f in fut:
                ...
                if s < m:
                    m = s
        res.append(m)
    out = (res, len(obs), len(fut), max(obs) if (obs and kind == "high") else (min(obs) if obs else None))
...
def main():
    ...
    print("Filtre VIVANT : prix entre %.0f%% et %.0f%% (le reste = deja resolu, ignore)\n"
    ...
    for m in mk:
        ...
        try:
            ...
            print("  (erreur prevision %s: %r)" % (city, e))
            continue
        live += 1
        mprob = prob_bucket(samples, lo, hi)
        edge = mprob - p
        info = "obs=%dh fut=%dh%s" % (nobs, nfut, ("" if omax is None else " max_obs=%.1fC" % omax))
        print("=" * 70)
        print("Q : %s" % m["q"][:80])
        ...
        if abs(edge) >= EDGE_MIN:
            ...
            print("  -> VALEUR: %s | f=%.1f%% (demi-Kelly plafonne) | mise %.1f EUR" % (side, f_used * 100, stake))
    ...
    print("\n%d marches VIVANTS analyses | %d paris de valeur (edge >= %.0f pts)."
          % (live, len(bets), EDGE_MIN * 100))
    if bets:
        state = {"created": datetime.now(timezone.utc).isoformat(timespec="seconds"),
                 "sigma_day": SIGMA_DAY, "bets": bets}
        with open(STATE, "w") as f:
            json.dump(state, f, indent=1)
        print("Paris journalises dans %s (a regler plus tard pour mesurer le win rate reel)." % STATE)
    print("\nRappel honnete: la source (Open-Meteo, grille) n'est PAS la station exacte de")
    print("resolution (aeroport/Wunderground). Un edge affiche peut n'etre que cet ecart de source.")
